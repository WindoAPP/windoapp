    ...
    case 'POST':
      try {
        ...
        if (
          ...
          return res
            .status(400)
            .send({ message: 'Incomplete or invalid data' });
        }

        const emailData: ContactMessageData = {
          ...data,
          email: process.env.NEXT_PUBLIC_FROM_EMAIL,
        }; // Update with the correct 'to' email address
